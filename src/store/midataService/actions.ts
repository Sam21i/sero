...
export async function uploadPendingResources(dispatch: Function): Promise<void> {
    ...
    uploadJobs.forEach((uploadJob: {resource : Resource, isUploading : boolean, mustBeSynchronized : boolean, timestamp: Date}) => {
...
export function uploadResource( dispatch: Function, _jobItem: {resource : Resource, isUploading : boolean, mustBeSynchronized : boolean, timestamp: Date}): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        if (!_jobItem.mustBeSynchronized || !_jobItem.resource.id) {
            ...
            if (Guid.isGuid(_jobItem.resource.id)) { // resource is in a I4MIBundle that has not been synced before resource was added to queue
...
export function deleteResource(dispatch: Function, _mustBeSynchronized: boolean, _resource: Resource): Promise<void> {
    return new Promise((resolve, reject) => {
        if (_resource.id) {
            if (Guid.isGuid(_resource.id)) { // this means resource is in a yet unsynced bundle
                const pendingJobs = ((store.getState()) as any).MiDataServiceStore.pendingResources;
                let foundResourceInPendingJobs = false;
                // look for the bundle if it's still in queue
                pendingJobs.forEach((pendingJob: { resource: Resource; }) => {
                ...
                store.getState().MiDataServiceStore.deleteResource(_resource).then(() => {
                    dispatch({
                        type: RESOURCE_SENT,
                        resource: {
                            resource: _resource,
